// 병원 관리자(직원) 계정 관리 — list / create / delete.  (멀티유닛/슈퍼관리자 지원)
// 보안: 호출자 JWT로 본인 확인 → 호출자가 관리하는 병원(clinic_admins, 여러 개 가능)만 대상.
//       body.clinicIds 로 대상 병원 지정하되, 호출자가 실제 관리하는 것만 허용.
//       service_role 은 Edge 에 자동 주입(클라 노출 0). 호출: db.functions.invoke('clinic-admin', {body:{action, clinicIds, ...}})
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 2) 호출자가 관리하는 병원들 (여러 개 가능 — 슈퍼관리자/멀티유닛)
    const { data: myRows } = await asUser.from('clinic_admins').select('clinic_id').eq('user_id', caller.id);
    const myClinics: string[] = (myRows ?? []).map((r: { clinic_id: string }) => r.clinic_id);
    if (!myClinics.length) return json({ error: '병원 관리자가 아닙니다' }, 403);

    const svc = createClient(SUPABASE_URL, SERVICE, { auth: { persistSession: false } });
    const body = await req.json().catch(() => ({}));
    const action = body.action ?? 'list';

    // 3) 대상 병원 = body.clinicIds 중 호출자가 실제 관리하는 것만 (없으면 전체)
    const requested: string[] = Array.isArray(body.clinicIds)
      ? body.clinicIds
      : (body.clinicId ? [body.clinicId] : myClinics);
    const targets = requested.filter((c) => myClinics.includes(c));
    ...
    if (action === 'delete') {
      ...
      // 대상 병원 매핑만 제거
      await svc.from('clinic_admins').delete().eq('user_id', targetId).in('clinic_id', targets);
